...
def _parse_candidate_input(text):
    ...
    # Extract skill names mentioned anywhere in the text
    ALL_KNOWN_SKILLS = (RETRIEVAL_SKILLS | LLM_SKILLS | ML_SKILLS |
                        {"Python","Docker","Kubernetes","AWS","GCP","Azure"})
    if not parsed["skills"]:
        found = [s for s in ALL_KNOWN_SKILLS if s.lower() in q]
        if found:
            parsed["skills"] = found
    # Extract title from "working as X" or "ai engineer" patterns
    if not parsed["title"]:
        m = re.search(r'working as (?:an? )?([a-z ]{3,30}?)(?:\.|,|$| in)', q)
        if m:
            parsed["title"] = m.group(1).strip().title()
...
/* Progress bars in score breakdown */
div[style*="border-radius:4px"] { transition: width 0.3s ease; }
</style>
""", unsafe_allow_html=True)
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "🏆 Top Candidates", "📊 Score Analysis", "💼 Pool Insights",
    "💬 Chat", "🔍 Evaluate"
])
...
with tab5:
    ...
    if submission is None:
        ...
        with st.form("candidate_eval_form", clear_on_submit=False):
            ...
            with c2:
                ev_github     = st.slider("GitHub Activity Score", min_value=-1,
                                          max_value=100, value=-1,
                                          help="-1 = not linked; 0–100 = activity score")
                ev_consulting = st.slider("Consulting Background (%)",
                                          min_value=0, max_value=100, value=0,
                                          help="% of career spent at outsourcing firms (TCS, Infosys…)")
            ...
            ev_skills = st.multiselect(
                ...
                options=_all_skills,
                default=[],
                help="Pick every skill the candidate has listed"
            )

            st.markdown("#### 📊 Evidence Signals")
            c3, c4 = st.columns(2)
            with c3:
                ev_has_eval = st.checkbox(
                    "✅ Has evaluation metrics in career history",
                    help="Mentions NDCG, MRR, MAP, A/B tests, offline eval, ranking quality in their work description"
                )
                if ev_has_eval:
                    ev_eval_detail = st.text_input(
                        "Brief detail (optional)",
                        placeholder="e.g. improved NDCG@10 by 14%"
                    )
            with c4:
                ev_has_prod = st.checkbox(
                    "✅ Has deployed ML systems to production",
                    help="Shipped, deployed, or currently running ML in production at scale"
                )
                if ev_has_prod:
                    ev_prod_detail = st.text_input(
                        "Brief detail (optional)",
                        placeholder="e.g. recommendation system serving 2M users"
                    )

            submitted = st.form_submit_button(
                "🚀 Evaluate Candidate", type="primary", width="stretch"
            )
